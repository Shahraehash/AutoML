...
export class RocChartComponent implements OnInit {
    ...
    ngOnInit() {
        this.rocChart('#roc', sampleData, this.rocChartOptions);
    }

    private rocChart(id, data, options) {
        const cfg = {
        ...
        function areaUnderCurve(input, tpr) {
        ...
        // draw the ROC curves
        function drawCurve(input, tpr, stroke) {
        ...
        function drawArea(input, tpr, fill) {
          svg.append('path')
            .attr('class', 'area')
            .attr('id', tpr + 'Area')
            .style({
