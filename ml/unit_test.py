...
def test_k_nearest_neighbor_with_standard_scaler():
    ...
    assert generalization['avg_sn_sp'] == 0.6795454545454545
...
def test_support_vector_machine():
    ...
    assert generalization['avg_sn_sp'] == 0.4920454545454545
...
def test_support_vector_machine_with_standard_scaler_and_roc_auc():
    ...
    generalization = generalize(model, pipeline[0], X2, Y2)
    assert generalization['accuracy'] == 0.7058823529411765
    assert generalization['avg_sn_sp'] == 0.6806818181818182
    assert generalization['f1'] == 0.6386395843174303
    assert generalization['sensitivity'] == 0.6363636363636364
